...
namespace MailServer.SSMTP
{
  public class SSMTPSession
  {
    ...
    private bool SwitchCommand(string ssmtpCommandtxt)
    {
      ...
      var argsText = ServerCore.GetArgsText(ssmtpCommandtxt, ssmtpCommand);
      // --------------------------------------------------------------------- //
    ...
    private void AUTH(string argsText)
    {
      ...
      switch (param[0].ToUpper())
      {
        ...
        case "LOGIN":
          ...
          authenticated = ServerCore.AuthUser(username, password);
    ...
    private void RCPT(string argsText)
    {
      ...
      if (!ServerCore.ValidateUser(recipientEmail))
      {
    ...
    private void DATA(string argsText)
    {
      ...
      var headers = Encoding.ASCII.GetBytes(header.ToCharArray());

      var replyCode = ServerCore.ReadData(clientSocket, out var reply, headers, ssmtpServer.MaxMessageSize,
          "\r\n.\r\n", ".\r\n");
      if (replyCode == ReadReplyCode.Ok)
      {
        ...
        using (MemoryStream msgStream = ServerCore.DoPeriodHandling(reply, false))
        {
          ...
          // store message
          using (var file = new FileStream("queue/" + sessionID + "_" + ex + "_" + Guid.NewGuid().GetHashCode().ToString("X") + "_Recent.txt", FileMode.Create, FileAccess.Write))
          {
            var bytes = new byte[msgStream.Length];
            msgStream.Read(bytes, 0, (int)msgStream.Length);
            file.Write(bytes, 0, bytes.Length);
            File.WriteAllBytes("domains/" + domain + "/" + username + "/Sent/" + sessionID + "_" + ex + "_" + Guid.NewGuid().GetHashCode().ToString("X") + "_Recent.txt", bytes);
            msgStream.Close();
            file.Close();
          }
    ...
    {
      var line = ServerCore.ReadLine(clientSocket, 500);
